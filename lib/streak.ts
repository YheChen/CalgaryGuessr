import { calgaryDateKey, daysBetweenKeys } from "@/lib/date-calgary";

/**
 * Daily-play streak, kept in localStorage.
 *
 * Device-local by design: there are no accounts, so a streak lives in one
 * browser and is lost when its storage is cleared. It is decoration, never an
 * input to scoring, so it does not matter that a player could edit it.
 *
 * Days are counted on the America/Edmonton boundary, the same one the daily
 * challenge uses, so the two cannot disagree about when today ended.
 */

const STORAGE_KEY = "cg_streak";
...
export interface StreakState {
  ...
  best: number;
  /** Calgary date key of the last recorded play, or null if never. */
 ...
 *
 * Pure, and idempotent for repeat plays on the same day, which is what stops a
...
export function advanceStreak(
  ...
  if (gap === 0) {
    ...
    // Stored date is in the future: a clock change, or a trip east across time
    // zones. Leave the run alone rather than punishing something the player did
    // not do.
    current = Math.max(1, state.current);
  } else {
...
/**
 * How a stored streak should be displayed given today's date. A run broken by
 * not playing shows as 0 rather than a stale number, without needing a write
 * to correct it.
 */
...
export function parseStreak(raw: string | null): StreakState {
  ...
  try {
    ...
    return {
      ...
      lastPlayedDate:
        typeof lastPlayedDate === "string" && lastPlayedDate
          ? lastPlayedDate
          : null,
    };
  } catch {
...
export function readStreak(now: Date = new Date()): StreakState {
  ...
  try {
    ...
    return displayedStreak(stored, calgaryDateKey(now));
  } catch {
...
export function recordPlayedToday(now: Date = new Date()): StreakState {
  ...
  try {
    const stored = parseStreak(window.localStorage.getItem(STORAGE_KEY));
    const next = advanceStreak(stored, calgaryDateKey(now));
