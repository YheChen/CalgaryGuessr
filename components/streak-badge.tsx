...
export function StreakBadge({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 rounded-full bg-calgary-gold/15 px-3 py-1.5 text-xs font-semibold text-calgary-gold ring-1 ring-inset ring-calgary-gold/30",
        className,
      )}
      title={
    ...
    >
      ...
      {days} in a row
      {showBest && value.best > value.current && (
        <span className="font-medium text-calgary-gold/70">
          best {value.best}
        </span>
      )}
    </span>
