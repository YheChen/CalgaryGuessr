...
export function MapThemeProvider({ children }: { children: ReactNode }) {
  ...
  //
  // Both the read and the write are guarded: touching window.localStorage
  // THROWS SecurityError when storage is blocked, it does not return null. This
  // provider wraps the whole app from the root layout, so an unguarded throw
  // here would take out every route rather than just the map toggle.
  useEffect(() => {
